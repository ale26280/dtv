	...
	<div class="span12">
		...
		<div class="widget-box">
			...
			<div class="widget-content nopadding">
				...
				{{ Form::open( array('url' => route('portfolio_editar_post', $registro['id']), 'method' => 'POST', 'files' => true, 'class' => 'form-horizontal') ) }} 
					...
					<div class="control-group {{ $errors->has( 'caetgoria_id' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::select( 'categoria_id', $categorias, Input::old('categoria_id') != "" ? Input::old('categoria_id'):$registro['categoria_id'] ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'subtitulo' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'subtitulo', Input::old( 'subtitulo' ) != "" ? Input::old( 'subtitulo' ):$registro['subtitulo'] ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'orden' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'orden', Input::old( 'orden' ) != "" ? Input::old( 'orden' ):$registro['orden'] ) }} <br/>
						...
						<div class="modal-body" style="height:400px; scroll:auto;">
							...
							@if ( count($fotos) > 0)
								...
								@foreach ($fotos as $foto)
									...
									<a href="javascript:void%200;" data-element="foto_{{$foto['id']}}" data-delete="{{ route('galeria_borrar', array('portfolio', 'foto', $foto['id']) ) }}" class="btn btn-mini deleteModal sec accion"><i class="icon-trash"></i> Eliminar foto</a>
							...
							<div id="dragandrophandler_fotos" class="dragandrophandler" data-tipo="fotos" data-route="{{ route('portfolio_foto_upload', $registro['id']) }}">Arrastre y suelte imagenes aqu&iacute; para cargar</div>
							<br/>
							<br/>
							<div id="status1"></div>
				...
				function sendFileToServer(formData,status,tipo,route) {
					...
					var jqXHR=$.ajax({
						...
						success: function(data){
							...
							// Agregamos la nueva imagen al DOM
							var li = $(document.createElement("LI"));
							li.html("<a href='javascript:void%200;' data-element='foto_"+ data.id +"' title='Eliminar' data-delete='/adm/portfolio/foto/"+ data.id +"/borrar' class='btn btn-mini deleteModal sec accion'><i class='icon-trash'></i> Eliminar foto</a>");
						...
						<div id="videosModal" class="modal hide fade" style="width:800px;">
							...
							<div class="modal-body" style="height:400px; scroll:auto;">
								...
								<div style='margin:15px' class="control-group {{ $errors->has('url') ? 'error' : '' }}">
									...
									<div class="controls">
										{{Form::text('video')}} {{ Form::button('Agregar', array('id' => 'addVideo', 'data-route' => route('portfolio_video_agregar', array($registro['id'])), 'data-elemento' => 'portfolio', 'data-tipo' => 'video' )) }}
										<span class="errNew">{{ $errors->first('descripcion') }}</span>
									</div>
								</div>
								...
								@foreach ( $videos as $video )
									...
									<a href="javascript:void%200;" data-element="video_{{ $video['id'] }}" data-delete="{{ route('galeria_borrar', array('portfolio', 'video', $video['id']) ) }}" class="btn btn-mini deleteModal sec accion"><i class="icon-trash"></i> Eliminar video</a>
									
									<img id="vidthumb_{{ $video['id'] }}" src="" width="220" height="auto" /></li>

									@if ($video['sitio'] == 'youtube')
				...
				<div class="form-actions">
					...
					<a href="{{ route('portfolio_listar') }}" class="btn"><i class="icon-remove-sign"></i> Cancelar</a>
