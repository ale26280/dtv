	...
	<div class="span12">
		...
		<div class="widget-box">
			...
			<div class="widget-content nopadding">
				...
				{{ Form::open( array('url' => route('ficha_editar_post', $registro['id']), 'method' => 'POST', 'files' => true, 'class' => 'form-horizontal') ) }} 
					...
					<div class="control-group {{ $errors->has('destacada') ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::checkbox('destacada', '1', $registro['destacada'] ) }} <br>
					...
					<div class="control-group {{ $errors->has('fecha') ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text('fecha', Input::old('fecha') != "" ? Input::old('fecha'):$registro['fecha'], array('id' => 'fecha_input') ) }} <br>
					...
					<div class="control-group {{ $errors->has( 'caetgoria_id' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::select( 'categoria_id', $categorias, Input::old('categoria_id') != "" ? Input::old('categoria_id'):$registro['categoria_id'], array('id' => 'comboCategoria', 'data-svc' => route('fichacategoria_tags'), 'data-parent' => $registro['id']) ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'imagen_principal' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							@if ($registro['imagen_principal'] != "")
							<img id="image_imagen_principal" src="{{ URL::to('/') }}/script/timthumb.php?src=/test/public/storage/fichas/{{ $registro['imagen_principal'] }}&w=180" /><br/>
					...
					<div class="control-group {{ $errors->has( 'presentacion' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							@if ($registro['presentacion'] != "")
							<a href="/test/public/storage/fichas/presentaciones/{{ $registro['presentacion'] }}" title="Ver archivo" target="_blank"><i class="icon-share"></i>Ver PDF Presentacion</a><br/>
					...
					<div class="control-group {{ $errors->has( 'titulo' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'titulo', $registro['titulo'] ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'cliente' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'cliente', Input::old( 'cliente' ) != "" ? Input::old( 'cliente' ):$registro['cliente'] ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'copete' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'copete', Input::old( 'copete' ) != "" ? Input::old( 'copete' ):$registro['copete'] ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'banner' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							@if ( $registro['banner'] != "")
							<img id="image_banner" src="{{ URL::to('/') }}/script/timthumb.php?src=/test/public/storage/fichas/{{ $registro['banner'] }}&w=180" /><br/>
					...
					<div class="control-group {{ $errors->has('ficha') ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::textarea('ficha', Input::old( 'ficha' ) != "" ? Input::old( 'ficha' ):$registro['ficha'], array('class' => 'tinymce')) }} <br>
					...
					<div class="control-group {{ $errors->has('info') ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::textarea('info', Input::old( 'info' ) != "" ? Input::old( 'info' ):$registro['info'], array('class' => 'tinymce')) }} <br>
					...
					<div class="control-group {{ $errors->has( 'web' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'web', Input::old( 'web' ) != "" ? Input::old( 'web' ):$registro['web'] ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'blog' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'blog', Input::old( 'blog' ) != "" ? Input::old( 'blog' ):$registro['blog'] ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'orden_home' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'orden_home', Input::old( 'orden_home' ) != "" ? Input::old( 'orden_home' ):$registro['orden_home'] ) }} <br/>
					...
					<div class="control-group {{ $errors->has( 'orden_interior' ) ? 'error' : '' }}">
						...
						<div class="controls">
							...
							{{ Form::text( 'orden_interior', Input::old( 'orden_interior' ) != "" ? Input::old( 'orden_interior' ):$registro['orden_interior'] ) }} <br/>
						...
						<div id="videosModal" class="modal hide fade" style="width:800px;">
							...
							<div class="modal-body" style="height:400px; scroll:auto;">
								...
								<div style='margin:15px' class="control-group {{ $errors->has('url') ? 'error' : '' }}">
									...
									<div class="controls">
										{{Form::text('video')}} {{ Form::button('Agregar', array('id' => 'addVideo', 'data-route' => route('ficha_video_agregar', array($registro['id'])), 'data-elemento' => 'ficha', 'data-tipo' => 'video' )) }}
								...
								@foreach ( $videos as $video )
									...
									<a href="javascript:void%200;" data-element="video_{{ $video['id'] }}" data-delete="{{ route('galeria_borrar', array('ficha', 'video', $video['id']) ) }}" class="btn btn-mini deleteModal sec accion"><i class="icon-trash"></i> Eliminar video</a>
					...
					<div class="control-group {{ $errors->has('fotos') ? 'error' : '' }}">
						...
						<div id="fotosModal" class="modal hide fade" style="width:800px;">
							...
							<div class="modal-body" style="height:400px; scroll:auto;">
								...
								<ul id="fotosGal"  class="imageGal sortable-list" data-update="{{ route('ficha_foto_reordenar') }}">
								...
								@if ( count($fotos) > 0)
									...
									@foreach ($fotos as $foto)
										...
										<a href="javascript:void%200;" data-element="foto_{{$foto['id']}}" data-delete="{{ route('galeria_borrar', array('ficha', 'foto', $foto['id']) ) }}" class="btn btn-mini deleteModal sec accion"><i class="icon-trash"></i> Eliminar foto</a>
										
										<img src="/test/public/storage/fotos/fichas/{{ $foto['fuente'] }}" width="220" height="auto" /></li>
									@endforeach
								...
								<div id="dragandrophandler_fotos" class="dragandrophandler" data-tipo="fotos" data-route="{{ route('ficha_foto_upload', $registro['id']) }}">Arrastre y suelte imagenes aqu&iacute; para cargar</div>
								<br/>
								<br/>
								<div id="status1"></div>
					...
					function sendFileToServer(formData,status,tipo,route) {
						...
						var jqXHR=$.ajax({
							...
							success: function(data){
								...
								// Agregamos la nueva imagen al DOM
								var li = $(document.createElement("LI"));
								li.html("<a href='javascript:void%200;' data-element='foto_"+ data.id +"' title='Eliminar' data-delete='/test/public/adm/ficha/foto/"+ data.id +"/borrar' class='btn btn-mini deleteModal sec accion'><i class='icon-trash'></i> Eliminar foto</a>");

								li.attr("id", "foto_"+ data.id);

								var img = new Image();
								img.src = '/test/public/storage/fotos/fichas/' + data.fuente;
								img.width = 220;
								img.onload = function() {
									li.append($(img));
